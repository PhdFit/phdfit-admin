...
// ---------------------------------------------------------------------------
// Shared node shape used by both mock and real data
// ---------------------------------------------------------------------------

export interface TaxonomyTreeNode {
  id: string;
  name: string;
  /** Display label for the badge, e.g. "domain", "topic", "method" */
  displayType: string;
  /** Key used for badge colour lookup */
...
function typeBadgeVariant(color: "domain" | "topic" | "method") {
  switch (color) {
    case "domain":
  ...
  return (
    <div>
      ...
      >
        ...
        <Badge
          variant="outline"
          className={`pointer-events-none border-transparent text-[10px] uppercase tracking-wide ${typeBadgeVariant(node.badgeColor)}`}
        >
          {node.displayType}
        </Badge>
...
export function TaxonomyTreeClient({
  tree,
  stats,
}: {
  ...
  // Expand / collapse state -- top-level nodes start expanded
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.methods}</div>
          </CardContent>
        </Card>
